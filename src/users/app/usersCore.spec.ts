...
describe('UsersCore', () => {
	...
	describe('createUser', () => {
		...
		it('debería lanzar BadRequestException si el usuario ya existe', async () => {
			...
			await expect(usersCore.createUser(validUser)).rejects.toThrow(BadRequestException);
		...
		it('debería lanzar TypeORMError si el repo falla', async () => {
			...
			await expect(usersCore.createUser(validUser)).rejects.toThrow(TypeORMError);
		});
	});
	...
	describe('getUsers', () => {
		...
		it('debería delegar los defaults si no se pasan parámetros', async () => {
			...
			expect(mockRepo.getUsers).toHaveBeenCalledWith({ skip: 0, take: 100 });
			expect(result).toStrictEqual(expectedResult);
		});
