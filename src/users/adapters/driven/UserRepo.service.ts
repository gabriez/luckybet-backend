...
export class UserRepoService implements ForDatabaseUsers {
	...
	async findToAuth(options: UserUniqueFields): Promise<UserWithMethods | null> {
		const result = await this.userModel.findOne({
			where: options,
			select: { password: true, id: true, username: true },
		});

		return result ? (result as UserWithMethods) : null;
	}

	async findByUnique(options: UserUniqueFields): Promise<UserWithoutPassword | null> {
		const result = await this.userModel.findOne({
			where: options,
			select: { id: true, role: true, username: true, isActive: true },
		});
